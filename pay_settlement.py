...
CHẠY TRÊN GITHUB ACTIONS — cấp qua biến môi trường, không cần file secret trong repo:
  TIKTOK_COOKIE_RHYSMAN        = nguyên chuỗi cookie (1 dòng)
...
def main():
    ...
    global SELLER_ID
    if TIKTOK_COOKIE:
        _src_name = _COOKIE_ENV if os.getenv(_COOKIE_ENV) else "TIKTOK_COOKIE"
        cookie, cookie_src = TIKTOK_COOKIE.strip(), f"ENV:{_src_name}"
    else:
        if not os.path.exists(COOKIE_FILE):
            sys.exit(f"[CONFIG] không thấy file {COOKIE_FILE}, "
                     f"và cũng không có ENV {_COOKIE_ENV} / TIKTOK_COOKIE.")
