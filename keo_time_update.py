...
DATASET_ID = "rhysman"
TABLE_ID = "fact_orders_salework_tiktok_updated_at"
...
def build_dataframe(orders):
    ...
    # Xóa dòng thiếu mã đơn
    df = df.dropna(subset=["code"])

    # Nếu bị trùng mã đơn thì giữ updatedAt mới nhất
    df = df.sort_values("updatedAt").drop_duplicates(
        subset=["code"],
        keep="last"
    )
...
def load_to_bigquery(df):
    ...
    print("✅ DONE LOAD CODE + UPDATEDAT TO BIGQUERY")
